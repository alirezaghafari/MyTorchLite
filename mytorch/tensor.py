...
class Tensor:
    ...
    def __setitem__(self, idcs, other):
        "Handle tensor item assignment."
        if not isinstance(other, Tensor):
            other = ensure_tensor(other)

        # Convert idcs to a tuple if it's not already
        if not isinstance(idcs, tuple):
            idcs = (idcs,)

        if len(idcs) != len(self.shape):
            raise IndexError("Number of indices must match tensor dimensionality")
        
        # Invalidate the gradient since the data is being modified
        self.grad = None
...
def _sub(t1: Tensor, t2: Tensor) -> Tensor:
    "Compute subtraction of two tensors"
    # Hint: a-b = a+(-b)
